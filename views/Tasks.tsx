      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-8 pt-8 pb-4">
          <div className="flex flex-wrap justify-between items-end gap-4">
            ...
            <button className="flex items-center justify-center gap-2 px-6 h-11 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold hover:border-primary transition-all shadow-sm">
        ...
        <div className="px-8 border-b border-slate-200 dark:border-slate-800">
          <div className="flex gap-8">
            <Tab active icon="view_list" label="Visualização em Lista" />
            <Tab icon="view_kanban" label="Visualização em Quadro" />
            <Tab icon="calendar_month" label="Cronograma" />
            <Tab icon="folder_open" label="Arquivos" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-x-auto p-8 bg-slate-50 dark:bg-slate-900/20">
          <div className="flex gap-6 min-w-max">
            <TaskColumn title="A Fazer" count={4}>
              <TaskCard title="Rascunho Identidade Visual - Style Scapes" category="Branding" date="24 Out" priority="Média" color="blue" />
              <TaskCard title="Mapeamento da Jornada do Usuário (Mobile)" category="Apps" date="26 Out" priority="Baixa" color="purple" />
            </TaskColumn>
            <TaskColumn title="Em Andamento" count={2} active>
              <TaskCard title="Wireframing - Home & Serviços" category="Web" sub="6/10 telas rascunhadas" date="Vence Amanhã" priority="Urgente" color="emerald" highlight />
            </TaskColumn>
            <TaskColumn title="Revisão Interna" count={1}>
              <TaskCard title="Setup do Backend - Integração CMS" category="Dev" sub="3 Comentários" date="28 Out" priority="Média" color="slate" />
            </TaskColumn>
          </div>
        </div>
      </div>
      ...
      <aside className="w-96 flex-shrink-0 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col hidden lg:flex">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Subtarefas (6/10)</span>
            <SubTask label="Layout do Header/Navegação" checked />
            <SubTask label="Conceitos da Seção Hero" checked />
            <SubTask label="Organização da Grade de Serviços" />
          </div>
        </div>
        <div className="p-4 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-800">
          <div className="flex gap-2 bg-white dark:bg-slate-900 p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <input className="flex-1 bg-transparent border-none text-xs focus:ring-0" placeholder="Escreva um comentário..." type="text"/>
            <button className="bg-primary text-white p-2 rounded-lg"><span className="material-symbols-outlined text-sm">send</span></button>
          </div>
        </div>
      </aside>
    </div>
...
const Tab: React.FC<{ active?: boolean; icon: string; label: string }> = ({ active, icon, label }) => (
  <button className={`flex items-center gap-2 border-b-2 pb-3 pt-4 px-1 ${active ? 'border-primary text-primary font-bold' : 'border-transparent text-slate-500 hover:text-slate-700 font-bold'}`}>
...
const TaskCard: React.FC<{ title: string; category: string; sub?: string; date: string; priority: string; color: string; highlight?: boolean }> = ({ title, category, sub, date, priority, color, highlight }) => (
  <div className={`bg-white dark:bg-slate-900 p-4 rounded-xl border ${highlight ? 'border-l-4 border-l-primary ring-1 ring-primary/5' : 'border-slate-200 dark:border-slate-800'} shadow-sm hover:border-primary/50 transition-all cursor-pointer group`}>
...
const SubTask: React.FC<{ label: string; checked?: boolean }> = ({ label, checked }) => (
  <div className={`flex items-center gap-3 p-3 rounded-lg border ${checked ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-200 shadow-sm'}`}>
